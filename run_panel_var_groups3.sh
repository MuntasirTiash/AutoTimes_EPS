...
# Feature groups JSON
VAR_GROUP_JSON="${ROOT_PATH}/var_group.json"

# LLaMA (updated to your local checkpoint under /largessd in this repo)
LLAMA_DIR="${REPO}/.models/llama-3.1-8b"

# Text embeddings index
TEXT_INDEX="${ROOT_PATH}/10-Q/embeddings_nasdow/gvkey_text_index.csv"
TEXT_DIM=4096
...
# ===================== MAIN LOOP (no skips; always both runs) =================
for KEY in "${KEYS[@]}"; do
  GROUP="$(sanitize "${KEY}")"
