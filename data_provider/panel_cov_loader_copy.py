...
class Dataset_PanelCov(Dataset):
    ...
    def _read_and_index(self):
        df = pd.read_csv(os.path.join(self.root_path, self.data_path),nrows = 10000)
        ...
        # ---- Standardize (train portions only), guard zero-variance
        if self.scale and len(self.series_X):
            train_chunks = []
            for X in self.series_X:
                n = len(X)
                n_train = int(0.7 * n)
                train_chunks.append(X[:n_train])
        ...
        # ---- Build (entity, start) index with validity checks
        self.index_pairs = []
        for i, (X, y) in enumerate(zip(self.series_X, self.series_y_raw)):
            n = len(X)
            n_train = int(0.7 * n)
            n_test = int(0.2 * n)
            n_val = n - n_train - n_test
            b1s = [0, n_train - self.seq_len, n - n_test - self.seq_len]
            b2s = [n_train, n_train + n_val, n]
            b1 = max(0, b1s[self.set_type]); b2 = b2s[self.set_type]
            max_start = b2 - self.pred_len - self.seq_len

            for s in range(b1, max_start + 1):
